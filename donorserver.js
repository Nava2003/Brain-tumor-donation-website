...
app.get("/api/donor/:username", async (req, res) => {
  try {
    const user = await Doctor.findOne({ username: req.params.username }, { password: 0 });
    if (!user) {
...
app.get("/api/donordetail/:username", async (req, res) => {
  const { username } = req.params;

  try {
    const donor = await Donor.findOne({ username }, { password: 0 });
    if (!donor) {
      return res.status(404).json({ error: "Donor not found" });
    }
