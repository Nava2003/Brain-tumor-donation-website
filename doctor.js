...
signin.addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('http://localhost:5003/signin', {
      method: 'POST',
      headers: {
...
forgotPasswordBtn.addEventListener("click", () => {
  // Show reset fields in signin form
  document.getElementById("new-password-field").style.display = "table-row";
  document.getElementById("reset-password-btn").style.display = "table-row";
});
...
async function handleResetPassword(event) {
  // Determine which form triggered the reset
  ...
  const newPassword = isFromSignin 
    ? document.getElementById("new-password").value.trim()
    : document.getElementById("new-password").value.trim();
  
  const newPasswordField = isFromSignin 
    ? document.getElementById("new-password-field-signin")
    : document.getElementById("new-password-field");
  
  const resetPasswordBtn = isFromSignin
  ...
  try {
    const response = await fetch("http://localhost:5003/reset-password", {
      method: "POST",
...
// Identity confirmed message function
function createIdentityConfirmedMessage() {
  ...
  setTimeout(() => {
    ...
    window.location.href = 'doctor.html'; // Redirect to donation page
  }, 2000);
}
