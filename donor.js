...
signin.addEventListener('click', async (e) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      sessionStorage.setItem('identityConfirmed', 'true'); // Prevent double flash
...
function createIdentityConfirmedMessage() {
  // Avoid duplicate flash on redirect
  if (sessionStorage.getItem('identityConfirmed') !== 'true') return;
  ...
  setTimeout(() => {
    ...
    window.location.href = 'give donation.html';
  }, 2000);
}
